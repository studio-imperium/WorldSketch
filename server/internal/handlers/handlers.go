...
func Config(w http.ResponseWriter, r *http.Request) {
	...
	_ = json.NewEncoder(w).Encode(map[string]any{
		...
		"generation": map[string]any{
			...
			"imageSpace":    config.Env("WS_HF_IMAGE_SPACE", "Qwen/Qwen-Image-Edit-2511"),
			...
			"image": map[string]any{
				...
				"width":    config.EnvInt("WS_HF_IMAGE_WIDTH", 1024, 512, 2048),
				"height":   config.EnvInt("WS_HF_IMAGE_HEIGHT", 1024, 512, 2048),
			},
			"tripo": map[string]any{
				"steps":     config.EnvInt("WS_HF_TRIPO_STEPS", 20, 1, 64),
				"guidance":  config.EnvFloat("WS_HF_TRIPO_GUIDANCE", 3),
				"gaussians": config.EnvInt("WS_HF_TRIPO_GAUSSIANS", 262144, 1024, 262144),
				"format":    "splat",
			},
		},
	})
