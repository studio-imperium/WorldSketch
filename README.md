...
**Image detail runs on Hugging Face inference credits by default** (`WS_HF_IMAGE_CREDITS=1`, ~1–2¢ per image via fal-ai): every ZeroGPU image route eventually died waiting in the GPU queue, so ZeroGPU is now only used for TripoSplat. Set `WS_HF_IMAGE_CREDITS=0` to route images through the Space below instead — the A/B lab follows the same switch.
