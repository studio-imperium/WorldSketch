...
The default image Space is `akhaliq/Qwen-Image-Edit-2509` — a two-image Qwen-Edit-Plus with the 8-step Lightning LoRA fused and only a 60s ZeroGPU reservation, so jobs are admitted even with little remaining quota. On this Space the sampler is fixed to Lightning's tuned numbers (8 steps, cfg 1), so `WS_HF_IMAGE_STEPS`/`WS_HF_IMAGE_GUIDANCE` do not apply. `WS_HF_IMAGE_SPACE` can point at the official `Qwen/Qwen-Image-Edit-2509` (unusable via API as of 2026-07-16: it reserves 600s per run, above every normal account's per-run ceiling), at `WilliamQM/Qwen-Image-Edit-2509` (a 120s-reservation duplicate, retired after its queue kept wedging), or back at `black-forest-labs/FLUX.2-dev` / `black-forest-labs/FLUX.2-klein-4B`.
