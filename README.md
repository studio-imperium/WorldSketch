...
Every app generation sends `client/assets/styleguide.png` as a **style guide** (the last input image; the prompt tells the model to copy its art style but take no objects or layout from it). Swap the file to change the app's look; if the asset is missing, generation proceeds without it. After generation, seating applies a **wisp cull** (fit.js): translucent gaussians stretching past a fraction of the scene's largest span are dropped outright. The "Remove floating wisps" slider (Overall cleanup group) controls the strength — 0 off, 50% = the original 4%-of-span threshold, 100% trims down to 1.5% — and retunes live like the other segmentation sliders.
...
The checked-in defaults are a production preset: a 1024×1024 image with 28 editing steps, 30 TripoSplat steps at guidance 3, and 131,072 Gaussians (`WS_HF_TRIPO_GAUSSIANS` can raise this to TripoSplat's 262,144 maximum). TripoSplat's BiRefNet background cutout (preprocessing) is ON by default; `WS_HF_TRIPO_PREPROCESS=0` suppresses it via the 1px transparent-border trick — the old behavior, for when the cutout carves away real terrain. ZeroGPU checks that the declared reservation fits within the remaining quota before starting, then accounts for the GPU time used.
